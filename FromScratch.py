...
class SpecularStreakScene:
    ...
    def setup_custom_lighting(self):
        # LIGHT SOURCE
        self.light_pos = np.array([0.0, 3.0, -60.0])
        ...
        self.material = {
            'ambient': np.array([0.01, 0.01, 0.01]),
            'diffuse': np.array([0.05, 0.05, 0.05]),  # low diffuse === wet surface ???
            'specular': np.array([1.0, 1.0, 1.0]),  # Very high specular
            'shininess': 128.0,  # Same shinyness asbefore
            'roughness': 0.05  # this is only for ward as I understand it
        }
        ...
        self.use_ward = False  # Set to False to use Blinn-Phong instead
